...
describe('GET /api/v1/profiles/:userId/follow', () => {
  ...
  describe('Bad request', () => {
    ...
    it('should throw an Error if user tries to follow himself/herself', async () => {
      try {
        const res = await chai.request(app)
          .post('/api/v1/auth/signup')
          .send(userDetails);
  ...
  describe('User not found', () => {
    it('should throw an Error if user being followed does not exist on database', async () => {
      try {
        const res = await chai.request(app)
          .post('/api/v1/auth/signup')
          .send(userDetails);
        ...
        decodedUserToken = jwt.decode(userToken.split('').reverse().join(''));
        const res2 = await chai.request(app)
          .get(`/api/v1/profiles/${decodedUserToken.id.slice(0, -12)}${'0'.repeat(12)}/follow`)
          .set('x-access-token', userToken);
  ...
  describe('Duplicate follow', () => {
    it('should throw an Error if that user is already being followed by follower', async () => {
      try {
        const res = await chai.request(app)
          .post('/api/v1/auth/signup')
          .send(userDetails);
